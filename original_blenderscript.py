...
pixelsPerUnit=100 # Image resolution. In STLs Units are not defined, so chose one you and your slicing software like.
desiredHeight=1 #enter value in the same unit
Offset=0

#Load the image: Option 1:
image_path=r"C:\Users\nicol\source\repos\Bildbearbeitung\heightmap.png"
json_path=r"C:\Users\nicol\source\repos\Bildbearbeitung\cookie_config.json"
...
def createFaces(imgarr):
    ...
    lowerface=[]
    indices=np.reshape(np.arange(0,w*h),(w,h))
    lowerface.extend(indices[0,:])
    lowerface.extend(indices[:,-1])
    lowerface.extend(np.flip(indices[-1,:]))
    lowerface.extend(np.flip(indices[:,0]))        
    faces.append(lowerface)  
    return faces    
...
def set_boundary_lower(imagearr):
    imagearr[0,:]=-1
    imagearr[-1,:]=-1
    imagearr[:,0]=-1
    imagearr[:,-1]=-1
...
def add_cube():
    ...
    bbox_corners = active_obj.bound_box[:]
    bbox_center = (bbox_corners[0][0] + bbox_corners[6][0]) / 2, (bbox_corners[0][1] + bbox_corners[6][1]) / 2, (bbox_corners[0][2] + bbox_corners[6][2]) / 2
    bbox_size = (abs(bbox_corners[6][0] - bbox_corners[0][0]), abs(bbox_corners[6][1] - bbox_corners[0][1]), abs(bbox_corners[6][2] - bbox_corners[0][2]))

    bpy.ops.mesh.primitive_cube_add(size=1)
    cube = bpy.context.active_object
    cube.dimensions = bbox_size
    cube.location = bbox_center
    cube.scale.z = 2.02 / cube.dimensions.z
    cube.location.z += cube.dimensions.z / 2 + 0.02+1

    cube.location.z -= cube.dimensions.z / 2 + 2
...
if __name__=="__main__":
    ...
    for obj in bpy.context.scene.objects:
        if obj.name.startswith("DepthObject"):
            obj.select_set(True)
    bpy.ops.object.delete()
    print("initial deletion done")   
    
    a=img2array(image,invert=invertImage)
    a=set_boundary_lower(a)
    ...
    #calculate scale
    current_max=get_max_size()
    target_size=max(a.shape)/parameters.get("ppmm")#scale: 
    print("cur",current_max,"target",target_size)
    
    print("adding boolean modifier")
    bpy.ops.object.modifier_add(type='BOOLEAN')
    bpy.context.object.modifiers["Boolean"].operation = 'DIFFERENCE'
    #bpy.context.object.modifiers["Boolean"].solver = 'FAST'
    cube_name=[obj.name for obj in bpy.context.scene.objects if obj.name.startswith("Cube") ][0]
    bpy.context.object.modifiers["Boolean"].object = bpy.context.scene.objects[cube_name]
    ...
    cutter_obj=bpy.context.scene.objects[0]
    cutter_obj.select_set(True)
    bpy.context.view_layer.objects.active = cutter_obj
    scale_object(target_size/current_max)
